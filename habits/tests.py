...
class HabitAPITests(APITestCase):
    ...
    def test_create_habit_with_invalid_periodicity(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(
            "Минимальная частота выполнения привычки — раз в 1 день, максимальная — раз в 7 дней.",
            response.data["non_field_errors"],
        )
    ...
    def test_no_reward_for_pleasant_habit(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(
            "Приятной привычке нельзя присваивать вознаграждение или связанную привычку.",
            response.data["non_field_errors"],
        )
